<%
/*###############################################################################
# Nombre del Programa :  SICMOR0205HHeader.jsp                                  #
...
# Compania            :  WELLCOM S.A. DE C.V.                                   #
# Proyecto/Procliente :  N-082253-12         Fecha:08/04/2013                   #
# Modificación        :                                                         #
#-----------------------------------------------------------------------------  #
#								MODIFICACIONES                                  #
# Autor               :  GERMAN GONZALEZ                                        #
# Compania            :  WELLCOM S.A. DE C.V.                                   #
# Proyecto/Procliente :  N-04-2207-13         Fecha:22/05/2014                  #
# Modificación        :                                                         #
#------------------------------------------------------------------------------#
...
<!-- INICIA MARCA DE CAMBIO WELLCOM N-08-2253-12 08/04/2013-->
<%@ page import="java.util.ArrayList" %>
<%@ page import="com.wellcom.prosa.sacii.*" %>
<!-- FIN MARCA DE CAMBIO WELLCOM N-08-2253-12 08/04/2013-->
...
<html>
        ...
        function validas(form1, accion)
        {
            if (accion == 0)
            {
                try
                {
                    ...
                    {
                        /* INICIA MARCA DE CAMBIO WELLCOM N-04-2204-13 22/05/2014 */
                        if (document.frmTOCHeader.txtfEndDate.value.substring(3, 5) == document.frmTOCHeader.txtfStartDate.value.substring(3, 5))
                        {
                            sendDatas205H('txtfStartDate', 'txtfEndDate', 'cuenta', 'comercio', 'referencia', 'tablePaint205H', 'tableResult', '0');
                        ...
                        {
                            alert("El rango de consulta debe de ser dentro del mismo mes");
                            return true;
                        }
                        /* FIN MARCA DE CAMBIO WELLCOM N-04-2207-13 22/05/2014 */
                    }
            ...
            {
                /* INICIA MARCA DE CAMBIO WELLCOM N-04-2207-13 22/05/2014 */
                if (document.frmTOCHeader.txtfEndDate.value.substring(3, 5) == document.frmTOCHeader.txtfStartDate.value.substring(3, 5))
                {
                    sendDatas205H('txtfStartDate', 'txtfEndDate', 'cuenta', 'comercio', 'referencia', 'tablePaint205H', 'tableResult', accion);
                    ventana(window.open('please.jsp', '', 'left=250,top=300,width=600,height=100,directories=0,location=0,menubar=0,resizable=1,scrollbars=1,status=0,titlebar=0,toolbar=0'));
                    return true;
                } else
                {
    ...
    <body>
        <div align="center">
            ...
            <form action="ControllerServletMonitH?action=SICMOR0205HMain" method="post" name="frmTOCHeader" target="mainFrameTOC" id="frmTOCHeader">
                    ...
                    <% 
                     String bandera=(String)session.getAttribute("mostrarTabla");
                      ...
                      //String table= (String)session.getAttribute("tabla0100" );
                              ...
                              out.println(tab.createTable(2050));
